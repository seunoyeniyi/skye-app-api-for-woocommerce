...
if (!function_exists('sk_get_simple_product_array')) {
    ...
    $the_return = array(
        ...
        // 'featured' => $product->get_featured(),
        // 'catalog_visibility' => $product->get_catalog_visibility(),
        'description' => ($hide_description) ? "" : $product->get_description(),
        // 'sku' => $product->get_sku(),
        // 'menu_order' => $product->get_menu_order(),
        // 'virtual' => $product->get_virtual(),
        // 'permalink' => get_permalink($product->get_id()),
...
if (!function_exists('sk_push_notification')) {
    function sk_push_notification($to, $data){
        ...
        $app_id = get_option( "sk_push_app_id", "");
        $url= SKYE_PUSH_PROVIDER == "onesignal" ? "https://onesignal.com/api/v1/notifications" : "https://fcm.googleapis.com/fcm/send";
        
        $fields_array = array('registration_ids'=>$to, 'notification'=>$data);
        if (isset($data["topic"])) {
            unset($fields_array["registration_ids"]);
            $fields_array['to'] = $data['topic'];
        }
        
        

        if (SKYE_PUSH_PROVIDER == "onesignal") {
            //if device is a string and not array - convert back to array, because onesignal accept only array
            if (!is_array($to)) {
                $to = explode(",", $to);
            }
            $fields_array = array(
            ...
            if (count($to) > 0) {
                $fields_array['include_player_ids'] = $to;
            } else {
                $fields_array["included_segments"] = array("Subscribed Users","Active Users", "Inactive Users");
            }
            //schedule
            if (isset($data["send_after"])) {
                $fields_array['send_after'] = $data["send_after"];
            }
            //picture
			if (isset($data["big_picture"])) {
                $fields_array['big_picture'] = $data["big_picture"];
            }
        }
